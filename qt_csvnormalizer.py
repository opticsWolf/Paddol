...
from PyQt5.QtWidgets import (
    ...
    QListWidgetItem, QMenu, QAction, QFileDialog, QLabel, QHBoxLayout, QLineEdit,
...
class ClickableLineEdit(QLineEdit):
    """A :class:`QLineEdit` that emits ``clicked`` when the widget is pressed."""
    onClicked = pyqtSignal()

    def mousePressEvent(self, event: QMouseEvent) -> None:
        """
        Emit :attr:`clicked` after handling the normal press event.
    
        Args:
            event (QMouseEvent): The mouse‑press event that triggered this call.
        """
        super().mousePressEvent(event)
        self.onClicked.emit()          # your slot will run here

class CSVNormalizerWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        def _make_baseline_row(label_text, key):
            """
            Helper to create a row with:
                [ QLabel | QLineEdit (read‑only) | QPushButton ]
            The button calls self._select_baseline(key).
            """
            hbox = QHBoxLayout()
        
            btn = QPushButton(label_text)
            btn.setIcon(QIcon(ICON_DICT['open_folder']))
            btn.clicked.connect(lambda _, k=key: self.select_baseline(k))
            ...
            #line_edit.onClicked.connect(self.on_baseline_selected)
            line_edit.onClicked.connect(lambda w=line_edit: self.on_baseline_selected(w))
            # Store a reference so we can update it later
            self.baseline_paths[key + '_edit'] = line_edit
            hbox.addWidget(line_edit, stretch=4)          

            return hbox

        # Add the two rows to the layout
        file_layout.addLayout(_make_baseline_row(" 0 % Baseline", '0'))
        file_layout.addLayout(_make_baseline_row(" 100 % Baseline", '100'))
    ...
    def select_baseline(self, key: str) -> None:
        """
        Open a QFileDialog for the baseline file with *key* ('0' or '100')
        and store the chosen path in `self.baseline_paths`.
        """
        options = QFileDialog.Options()
        # You can tweak filters here; by default we allow any CSV.
        file_path, _ = QFileDialog.getOpenFileName(
            self,
            f"Select {key} % Baseline CSV",
            "",
            "CSV Files (*.csv);;TXT Files (*.txt);;All Files (*)",
            options=options
        )
        if file_path:
            # Store the path and update the UI line‑edit
            self.baseline_paths[key] = file_path
            edit: QLineEdit = self.baseline_paths[key + '_edit']
    ...
    def on_baseline_selected(self, widget: ClickableLineEdit) -> None:
        """Slot that receives the clicked ``ClickableLineEdit``.
    
        The function first verifies that the path actually exists; if it does
        not a warning dialog is shown and the preview step is skipped.
        """
