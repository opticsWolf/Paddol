...
class ZoomPipette(QWidget):
    """
    The visual overlay widget that follows the mouse and provides a zoomed view
    of the pixels underneath.
    
    Attributes:
        diameter (int): The total pixel width/height of the widget.
        zoom_outer (float): Zoom level for the outer ring.
        zoom_inner (float): Zoom level for the center reticle.
    """
    ...
    def refresh_snapshot(self) -> None:
        ...
        Hides the widget momentarily to avoid capturing itself.
        """
        pos = QCursor.pos()
    ...
    def _calculate_source_rect(self, zoom_factor: float, diameter_px: int) -> QRect:
        """Calculates the rectangle on the screenshot that corresponds to the zoomed view."""
    ...
    def _sync_sample_color(self) -> None:
        """Extracts the pixel color at the exact center of the current lens."""
    ...
    def _tick(self) -> None:
        """Timer callback to sync position with cursor."""
        self.update_position(QCursor.pos())

    # --- Event Handlers ---

    def mousePressEvent(self, event: QMouseEvent) -> None:
        """Capture clicks directly."""
...
class PixelColorPicker(QObject):
    ...
    def pick() -> Optional[QColor]:
        """
        Launches the modal color picker.
        
        Returns:
            QColor: The selected color.
            None: If the operation was cancelled (ESC or Right Click).
        """
